...
router.put('/update_phase/:phaseId', async (req, res) => {
  ...
  // Convert data to numbers if provided
  const convertedHours = hours !== undefined ? parseFloat(hours) : undefined;
  const convertedLiveSessions = live_sessions !== undefined ? parseFloat(live_sessions) : undefined;
  const convertedAssessments = assessments !== undefined ? parseFloat(assessments) : undefined;
  const convertedMockInterviews = mock_interviews !== undefined ? parseFloat(mock_interviews) : undefined;
  const convertedWebinars = webinars !== undefined ? parseFloat(webinars) : undefined;
  const convertedStudentReviews = student_reviews !== undefined ? parseFloat(student_reviews) : undefined;

  // Build the SET clause of the UPDATE query dynamically
  const updateFields = [];
  const updateValues = [];
  if (title !== undefined) { updateFields.push('title = $1'); updateValues.push(title); }
  if (start_date !== undefined) { updateFields.push('start_date = $2'); updateValues.push(start_date); }
  if (end_date !== undefined) { updateFields.push('end_date = $3'); updateValues.push(end_date); }
  if (convertedHours !== undefined) { updateFields.push('hours = $4'); updateValues.push(convertedHours); }
  if (convertedLiveSessions !== undefined) { updateFields.push('live_sessions = $5'); updateValues.push(convertedLiveSessions); }
  if (convertedAssessments !== undefined) { updateFields.push('assessments = $6'); updateValues.push(convertedAssessments); }
  if (convertedMockInterviews !== undefined) { updateFields.push('mock_interviews = $7'); updateValues.push(convertedMockInterviews); }
  if (convertedWebinars !== undefined) { updateFields.push('webinars = $8'); updateValues.push(convertedWebinars); }
  if (convertedStudentReviews !== undefined) { updateFields.push('student_reviews = $9'); updateValues.push(convertedStudentReviews); }
  ...
  // Add the phase ID to the update values
  ...
  try {
      ...
      const existingPhaseResult = await pool.query('SELECT id FROM report.phase WHERE id = $1', [phaseId]);
      if (existingPhaseResult.rows.length > 0) {
