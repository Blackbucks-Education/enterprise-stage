...
SELECT DISTINCT
    ...
    es.BTechBranch,
    es.YOP,
    round(ps1.total_score, 2) AS total_score,
    round(ps1.aptitude, 2) AS aptitude,
    round(ps1.coding, 2) AS coding,
    round(ps1.english, 2) AS english,
    ps1.employability_band,
    ps1.possible_employability_band,
    ...
    ps1.comment,
    ps1.aptitude_improvement_suggestions,
    ps1.technical_improvement_suggestions,
    ps1.english_improvement_suggestions,
    cs.codingScore,
    cs.codingAccuracy,
    cs.javaScore,
    cs.javaAccuracy,
...
LEFT JOIN
    coding_scores cs ON u.id = cs.user_id
WHERE
    c.id = $1`;
    ...
    sqlQuery += ' ORDER BY total_score DESC;';
...
router.get('/emp_band_data', isAuthenticated, async (req, res) => {
    try {
        ...
        // Generate the SQL query based on the parameters
