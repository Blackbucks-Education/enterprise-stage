...
group by pls.phase_id)
  ...
  FROM
      phase_data pd
  LEFT JOIN
      assessment_data ad  ON  pd.id = ad.phase_id
  LEFT JOIN live_sessions_data lsd ON  pd.id = lsd.phase_id
  LEFT JOIN EventFeedback ef on pd.id = ef.phase_id
  where pd.training_id = $1;
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Details are not found.' });
    }
...
LIMIT
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Top Students are not found.' });
    }
...
router.get('/assessments/:id',isAuthenticated, async (req, res) => {
  const trainingId = req.params.id;
