...
PhaseAttendance AS (
    ...
    FROM
        ...
        INNER JOIN report.phase_batch pb ON p.id = pb.phase_id
        INNER JOIN report.batch_data bd ON pb.batch_id = bd.batch_id AND sr.registration_number = bd.regno
    GROUP BY
        pls.phase_id
),
LiveSessionHours AS (
...
WHERE
    p.id = $1 -- Filter to get students who are absent for all events
GROUP BY
...
router.get('/liveSessions_syllabus/:id', async (req, res) => {

  const phaseId = req.params.id;

  try {
      // SQL query to join `live_sessions` and `faculties` tables
      const query = `
       SELECT e.event_title, f.name, fs.session_details, fs.session_materials
FROM report.events e
LEFT JOIN report.faculties f ON e.faculty_id = f.id
LEFT JOIN report.faculty_sessions fs ON e.id = fs.event_id
LEFT JOIN report.phase_live_sessions pls ON e.id = pls.event_id
WHERE pls.phase_id = $1
ORDER BY e.created_at ASC;
      ...
      if (result.rows.length === 0) {
        return res.status(404).json({ error: 'No data found for syllabus' });
      }
    ...
    const query = `
      ...
      hackathon_scores AS (
          ...
          FROM
              report.batch_data bd
          LEFT JOIN "user" u ON bd.email = u.email
          LEFT JOIN user_hackathon_participation uhp ON u.id = uhp.user_id
      ...
      SELECT
          ...
          sd.email,
          sd.regno,
        COALESCE(hs.test_score, 0) as test_score,
          COALESCE(hs.hackathon_scores, '{}'::jsonb) AS hackathon_scores  -- Return an empty JSON object if no scores
      FROM
          student_data sd
      LEFT JOIN hackathon_scores hs ON sd.email = hs.email;  -- Left join to include all students
`;
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ 
          error: 'The test has not been conducted yet, so there are no results to show.\nPlease stay tuned for updates.' 
      });
  }
...
router.get('/data/:id', async (req, res) => {
  try {
    ...
    // Query to get all events for the phase
    const eventsQuery = `
      SELECT event_title
      FROM report.events e
    ...
    // All available events in the phase
    const allEvents = eventsResult.rows.map(row => row.event_title);

    // SQL to get student data along with attended events based on student_responses
    const sql = `
    ...
    SELECT
        ...
        json_agg(DISTINCT CASE
            WHEN sr.event_id IS NOT NULL THEN e.event_title
            ELSE NULL
        END) FILTER (WHERE sr.event_id IS NOT NULL) AS attended_events -- Only include attended events
    FROM
        report.batch_data bd
    INNER JOIN
    ...
    LEFT JOIN
        report.student_responses sr ON sr.registration_number = bd.regno AND sr.event_id = e.id -- Include student responses where event matches
    WHERE
        p.id = $1
    GROUP BY
        bd.regno, c.code, bd.name, bd.email, bd.phone
)
SELECT
    name,
    email,
...
FROM
    ...
    // Process the results to build the response data
    const data = result.rows.map(row => {
      const attendedEvents = row.attended_events || [];
      const rowData = {
      ...
      // For each event in the phase, check if the student attended
      allEvents.forEach(event => {
        rowData[event] = attendedEvents.includes(event) ? '✔' : '❌';
      });
...
router.get('/assessments_inactive_students/:phaseId',isAuthenticated, async (req, res) => {
  ...
  try {
      ...
      if (rows.length === 0) {
        return res.status(404).json({ error: 'No inactive students to show at this time, as the tests have not yet begun' });
      }
  ...
  try {
      const overviewQuery = `
          ...
          SELECT
              ...
              ROUND(ef.average_feedback,2) As average_feedback,
