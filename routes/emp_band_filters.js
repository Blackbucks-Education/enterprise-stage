...
router.get('/emp_band_data', isAuthenticated, async (req, res) => {
    try {
        ...
        if (year) {
            const yearsArray = year.split(','); // Split the year parameter
            const yearPlaceholders = yearsArray.map((_, index) => `$${queryParams.length + index + 1}`).join(', ');
            filterConditions.push(`EXTRACT(YEAR FROM education.end_date) IN (${yearPlaceholders})`);
            queryParams.push(...yearsArray); // Add years to query params
        }
