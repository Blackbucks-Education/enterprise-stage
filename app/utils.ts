...
export function convertToBranchData(data) {
  const branchMap = {
  ...
  // Edge case: Check if data is an array
  if (!Array.isArray(data)) return [];

  // Calculate total enrolled candidates
  const totalCandidates = data.reduce((sum, item) => {
    if (item?.btechbranch && item.btechbranch !== 'Not Provided') {
      return sum + parseInt(item.total_enrolled_candidates, 10) || 0;
    }
    return sum;
  }, 0);
  ...
  const transformedData = data
    ...
    .map(item => ({
      ...
      value: parseInt(item.total_enrolled_candidates, 10) || 0,
      percentage: totalCandidates > 0
        ? ((parseInt(item.total_enrolled_candidates, 10) / totalCandidates) * 100).toFixed(2)
        : "0.00",
    }))
...
export function getTop3DataWithConfig(data) {
  ...
  // Edge case: Check if data is an array
  if (!Array.isArray(data)) return { chartData: [], chartConfig: {} };

  // Filter out entries where yop is 'Not Provided'
  const filteredData = data.filter(item => item?.yop && item.yop !== 'Not Provided');

  // Calculate total number of candidates
  const totalCandidates = filteredData.reduce((sum, item) => sum + (parseInt(item.total_enrolled_candidates, 10) || 0), 0);

  // Edge case: Avoid division by zero
  if (totalCandidates === 0) return { chartData: [], chartConfig: {} };

  // Calculate percentages and sort by total_enrolled_candidates in descending order
  const top3Data = filteredData
    .map(item => ({
      yop: item.yop,
      students: parseInt(item.total_enrolled_candidates, 10) || 0,
  ...
  const chartData = top3Data.map((entry, index) => ({
    ...
    fill: colors[index] || "#000", // Fallback color
...
export function generateChartDataAssessments(data) {
  ...
  // Define colors for the top 3 items
  const colors = ["#C084FC", "#CCF6A4", "#FB923C"];

  // Edge case: Avoid division by zero
  const totalTop3Count = top3.reduce((acc, item) => acc + (parseInt(item.count, 10) || 0), 0);
  if (totalTop3Count === 0) return { chartData: [], chartConfig: {} };

  // Create chartData
  const chartData = top3.map((item, index) => ({
    browser: item.name || "Unknown",
    students: parseInt(item.count, 10) || 0,
    fill: colors[index] || "#000", // Fallback color
    show: `${((parseInt(item.count, 10) / totalTop3Count) * 100).toFixed(2)}%`
  }));

  // Create chartConfig
  const chartConfig = top3.reduce((config, item, index) => {
    config[item.name || "Unknown"] = {
