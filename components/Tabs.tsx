...
interface TabsProps {
  company: string;
  noMockPapers: boolean;
}

export default function Tabs({ company, noMockPapers }: TabsProps) {
  const searchParams = useSearchParams();
  const tab = searchParams.get("tab") || "overview";
   // Default to 'overview' if tab is null
  let routes= [];
  if (noMockPapers) {
     routes = [
      {
      ...
      {
        mainpath: `/company?company=${company}&tab=interview`,
        label: "Interviews",
        path: "interview",
      },
      {
        mainpath: `/company?company=${company}&tab=pattern`,
        label: "Exam Pattern and Syallabus",
        path: "pattern",
      },
    ];
  } else {
    routes = [
      ...
      {
        mainpath: `/company?company=${company}&tab=questions`,
        label: "Mock Papers and Questions",
        path: "questions",
      },
    ];
  }
