...
const defaultTotalTests = 9464;
// let finalTotalTests = defaultTotalTests;
...
export function AssessmentsPar({totalTests}) {
  ...
  const generateRandomData = (finalTotalTests) => {
    const data = [];
    const now = new Date();
    ...
    let remainingTests = finalTotalTests;
  ...
  return (
    <Card className="w-full bg-white h-[22rem] md:w-[47%]" 
    style={{
      opacity: chartData?.length > 0 ? 1 : 0.5
    }}
    >
      <CardHeader className="flex items-center gap-2 space-y-0 border-b py-5 sm:flex-row">
        <div className="grid flex-1 gap-1 text-center sm:text-left">
          <CardTitle className="text-lg text-opacity-70 font-semibold">Test Participation</CardTitle>
        </div>
