...
const EmployabilityChart = () => {
  ...
  //Adding default Data for Not found handling
  const [state,setState] = useState({
      isAssessmentDataAvailable : true,
      isEmployabilityDataAvailable :true,
  })
  ...
  const defaultLabels = ["A++", "A+", "B", "C", "F"];
  
  const dummyEmployabilityData = defaultLabels.map((label) => ({
    label,
    empCount: defaultEmployabilityData.emp_band_counts[label] || 0,
    bestCount: defaultEmployabilityData.best_band_counts[label] || 0,
  }));
  ...
  useEffect(() => {
    ...
    fetch(apiUrl)
      ...
      .catch((error)=>{
        ...
        setState((prevState)=>({
          ...prevState,
          isAssessmentDataAvailable : false,
        }))
      });
  }, []);
  ...
  const fetchData = async () => {
    try {
      ...
      setState((prevState)=>({
        ...prevState,
        isEmployabilityDataAvailable : false,
      }))
    }
  };
  ...
  return (
    <div className="p-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 md:grid-cols-1 gap-5">
        <Card   className="bg-white" style={{ opacity: state.isEmployabilityDataAvailable ? 1 : 0.5 }}>
          <CardHeader>
          ...
          <CardContent>
            <ChartContainer config={chartConfig}>
              <AreaChart
                data={state.isEmployabilityDataAvailable ? data : dummyEmployabilityData}
                margin={{
                  left: 12,
                  right: 12,
                }}
        ...
        <Card className="bg-white" style={{ opacity: state.isEmployabilityDataAvailable ? 1 : 0.5 }}>
          ...
          <CardContent >
            <ChartContainer config={chartConfig1}>
              <BarChart barSize={35} accessibilityLayer data={state.isAssessmentDataAvailable ? chartData1 : defaultAssessmentData}>
